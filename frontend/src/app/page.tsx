...
export default function Home() {
  ...
  const [loginRole, setLoginRole] = useState<'mentor' | 'admin' | ''>('');
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (activePortal === 'mentor' && !loginRole) {
    ...
    try {
      const data = await apiRequest('/auth/login', {
        ...
        body: JSON.stringify({
          username: loginUser,
          password: loginPass,
          portal: activePortal === 'student' ? 'student' : loginRole,
          rememberMe
        })
      });
      setCurrentUser(data.user, data.token);
          ...
          <div className="relative w-full max-w-lg bg-white border border-slate-200 rounded-3xl p-8 shadow-2xl z-10 text-left animate-in fade-in zoom-in-95 duration-200 overflow-y-auto max-h-[90vh]">
              ...
              <p className="text-xs text-slate-550 font-bold">
                {authMode === 'login' && (
                  activePortal === 'student'
                    ...
                    : 'Log in to access your Mentor or Admin workspace'
                )}
            ...
            {authMode === 'login' && (
              <form onSubmit={handleLogin} className="space-y-4">
                  ...
                  {activePortal === 'mentor' && (
                    ...
                    >
                      ...
                      <div className="relative" ref={roleDropdownRef}>
                        <UserCheck className="absolute left-3 top-2.5 h-4 w-4 text-slate-400 pointer-events-none z-10" />
                        <button
                          type="button"
                          onClick={() => setShowRoleDropdown(!showRoleDropdown)}
                          className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-10 py-2.5 text-xs text-slate-800 placeholder-slate-400 focus:border-[#0E3E31]/50 focus:bg-white outline-none transition-all flex items-center justify-between text-left h-[38px] cursor-pointer"
                        >
                          <span className={loginRole ? "text-slate-800 font-medium" : "text-slate-400"}>
                            {loginRole ? LOGIN_ROLES.find(r => r.value === loginRole)?.label : "Select your role"}
                          </span>
                          <svg
                            className={`w-3.5 h-3.5 text-slate-400 transition-transform duration-200 ${showRoleDropdown ? 'rotate-180' : ''}`}
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                          >
                        ...
                        {showRoleDropdown && (
                          <div className="absolute left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-xl z-[99] py-1 overflow-hidden animate-in fade-in slide-in-from-top-1 duration-150">
                            {LOGIN_ROLES.map((role) => (
                              <button
                                key={role.value}
                                type="button"
                                onClick={() => {
                                  setLoginRole(role.value as 'mentor' | 'admin');
                                  setShowRoleDropdown(false);
                                }}
                                className={`w-full px-4 py-2 text-left text-xs font-medium transition-colors hover:bg-slate-50 ${loginRole === role.value ? 'text-[#0E3E31] bg-[#E6F2ED]' : 'text-slate-700'}`}
                              >
                                {role.label}
                              </button>
                            ))}
                ...
                <button
                  type="submit"
                  disabled={formLoading || !loginUser || !loginPass || (activePortal === 'mentor' && !loginRole)}
                  className="w-full py-3 bg-[#0E3E31] hover:bg-[#0B3026] disabled:bg-[#0E3E31]/50 text-white rounded-xl text-xs font-extrabold shadow-lg shadow-[#0E3E31]/10 transition-all flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
                >
                  {formLoading && <RefreshCw className="h-3 w-3 animate-spin" />}
                  Log In to {activePortal === 'student' ? 'Student' : loginRole === 'mentor' ? 'Mentor' : loginRole === 'admin' ? 'Admin' : 'Mentor/Admin'} Desk
                </button>
